        ...
        {colors.map((colorItem) => (
          <div key={colorItem.id} className="flex flex-col">
            <div className="flex items-center gap-2 py-0.5">
              <div className="w-4 h-4 rounded" style={{ backgroundColor: `${colorItem.color}` }} />
              <input min={0} max={1} step={0.1} type="range" onChange={(e) => dispatch({ type: "SET_OFFSET", payload: { id: colorItem.id, offset: Number(e.target.value) } })} />
            </div>
          </div>
        ))}
      </div>
